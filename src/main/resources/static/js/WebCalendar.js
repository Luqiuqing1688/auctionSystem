...
function selectDate(obj, strFormat) {
    ...
    //最大值 → 50 年后
    cal = new Calendar(by, ey, 0, strFormat);    //初始化1英文版，0 为中文版
    cal = (cal == null) ? new Calendar(by, ey, 0) : cal;
    //不用每次都初始化 2006-12-03 修正
...
Calendar.prototype.draw = function () {
    ...
    var mvAry = [];
    //mvAry[mvAry.length]  = '  <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 2006-12-01 由*改用 Div
...
Calendar.prototype.bindYear = function () {
    ...
    //2006-12-01 由*修改
    cy.length = 0;
    for (var i = this.beginYear; i <= this.endYear;
         i++) {
...
Calendar.prototype.show = function (dateObj, popControl) {
    ...
    //由* 2006-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件
...
或
txtDate.onclick = function() {selectDate(this,'yyyy-MM-dd');};
或
